...
| F13 | P1 | **Partiellement corrigé localement :** les filtres Flatpak couvrent maintenant stockage, tracker, capacités, tests et requirements ; le workflow release ne dépend toujours pas d’un job CI externe et doit lancer/attacher tous ses audits de manière explicite. | `.github/workflows/flatpak.yml`, `release.yml`, `ci.yml` |
| F14 | P1 | **Partiellement corrigé localement :** le smoke échoue désormais si `xvfb-run` manque ou si la désinstallation `--delete-data` échoue, et le rapport de permissions rejette les filesystems larges ; le résultat JSON par étape reste à construire. Le script ne doit pas être lancé sur le profil quotidien d’un utilisateur : son cleanup supprime les données de l’app. | `packaging/smoke_test_flatpak.sh`, `scripts/run_release_tests.py` |
