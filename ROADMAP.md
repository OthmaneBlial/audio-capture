...
la vérification. Le workflow Flatpak GitHub Actions `34255478952` a réussi sur
le commit `cee8dc8` le 8 septembre 2026 : build en ligne, rebuild sans
...
une validation Orca, micro physique, traduction Groq ou machine utilisateur.
...
- Workflows Flatpak, Release, benchmark et CodeQL actifs. CodeQL et le smoke
  Flatpak ont réussi sur le dernier commit de code `cee8dc8` (run
  `34255478952`) ; les commits suivants sur `main` ne changent que la
  documentation. Ces jobs ne remplacent pas les
...
| F10 | P1 | **Corrigé localement après l’audit :** Settings expose un test micro local qui met à jour la jauge sans clé, VAD ni file audio, puis libère le périphérique à l’arrêt ; l’essai réel sur un bureau reste à valider. | `main.py`, `audio/capture.py`, `ui/main_window.py`, README |
...
| F12 | P1 | **Partiellement corrigé localement :** les tests purs ont un fichier de dépendances séparé et le guide calcule maintenant une couverture cœur/contrôleur sans UI native ; le contrôle GTK et le rapport de non-persistance raccordé aux écritures réelles restent ouverts. | `.github/workflows/ci.yml`, `requirements-test.txt`, `tests/test_privacy.py`, `tests/gtk_accessibility_smoke.py` |
| F13 | P1 | **Corrigé localement pour le chemin automatisé :** les filtres Flatpak couvrent maintenant stockage, tracker, capacités, tests et requirements ; le workflow release exécute un job qualité séparé (suite, couverture, Ruff, compilation, pip-audit et Bandit) avant le build et la publication. La porte humaine et la décision sur la CI générale restent ouvertes. | `.github/workflows/flatpak.yml`, `release.yml`, `ci.yml` |
| F14 | P1 | **Corrigé localement pour le chemin automatisé :** le smoke échoue si `xvfb-run` manque ou si la désinstallation `--delete-data` échoue, le rapport de permissions rejette les filesystems larges et le rapport JSON expose un statut explicite par étape. Le script ne doit pas être lancé sur le profil quotidien d’un utilisateur : son cleanup supprime les données de l’app ; le bureau physique, l’export portail et la recette téléchargée restent ouverts. | `packaging/smoke_test_flatpak.sh`, `scripts/run_release_tests.py` |
...
| F16 | P1 | **Partiellement corrigé localement :** `requirements-test.txt` et les guides fournissent maintenant une boucle sans dépendances GTK/PortAudio, tandis que `requirements-dev.txt` reste le chemin complet. Les ancres historiques de `CONTRIBUTING.md` et l’installation source doivent encore être revérifiées sur environnement vierge. | `requirements-test.txt`, `requirements-dev.txt`, `README.md`, `CONTRIBUTING.md`, `docs/contributing/*` |
...
- [x] Smoke Flatpak Linux vérifié sur `cee8dc8` (`34255478952`), avec suppression des données.
...
- **Changements :** identifiants de session et de segment dans les résultats ; ordonnancement borné ; définition des états terminé/échoué/annulé/vide ; invalidation des résultats tardifs à l’abandon ou au remplacement du provider. Une erreur de A doit libérer B sans attente infinie. **Fait localement :** un tampon partagé libère les résultats asynchrones dans l’ordre de soumission tout en conservant des workers parallèles ; les callbacks de résultat et d’erreur portent `request_id`, le contrôleur ignore ceux qui ne sont plus actifs, les annulations tardives ne peuvent plus recréer une ligne après Clear/reset, la fixture provider reproduit le même contrat et le bandeau des segments est vidé à chaque nouvelle génération. L’annulation/abandon complet et la génération persistante de session restent ouverts.
...
- **Changements :** Stop ferme le micro, traite les trames déjà admises et attend les résultats utiles avec état « traitement restant » ; Cancel invalide et supprime le travail annulable sans flush cloud. Ajouter une fermeture coordonnée avant destruction des widgets. Distinguer effacement réversible et abandon définitif ; pour ce dernier purger Undo/Redo et les résultats en attente. **Fait localement :** le bouton Clear invalide la génération active, annule les requêtes encore en file, n’enregistre plus de snapshot Undo et purge les piles Undo/Redo ; Stop conserve maintenant la file audio jusqu’au drain VAD de la dernière trame, affiche « Processing remaining… » lorsqu’un résultat reste en vol, puis revient à « Transcript ready to review » après le dernier résultat ; les callbacks portent un garde-fou `request_id`. La fermeture coordonnée et la mesure du backlog restent ouvertes.
...
- **Tests/validations :** tests locaux de Clear/Undo, reset de génération et retour tardif ignoré ; restent à valider stop avec backlog audio, fermeture pendant HTTP lent, SIGTERM, copie automatique, délais de fermeture et blocage du lecteur audio.
...
- **Changements :** respecter `XDG_CONFIG_HOME` avec repli et migration prudente ; préparer un nouvel état puis ne le valider en mémoire qu’après sauvegarde ; normaliser tous les échecs d’I/O. **Fait localement :** le chemin par défaut respecte maintenant `XDG_CONFIG_HOME`, les erreurs d’écriture de configuration sont converties en `ConfigError`, une mise à jour échouée restaure l’état mémoire, l’historique refuse les JSON corrompus/futurs sans les écraser et borne son volume, et les exports sont écrits atomiquement en refusant une destination symbolique. Le portail Flatpak et la migration restent ouverts.
...
- **Tests/validations :** tests locaux XDG, rollback, JSON invalide/schéma futur, volume historique, disque plein simulé pour export et destination symlink ; restent à valider dossier non inscriptible, migration, export via portail et nettoyage de données après désinstallation.
...
- **Changements :** commande unique de validation locale ; alignement des contrôles release et CI ; filtres Flatpak couvrant tous les modules runtime, tests et outils de release ; pinning des actions/images critiques par révision/digest avec mise à jour contrôlée ; examen des PR Dependabot. Formaliser le choix mainteneur sur la pause et le canal privé de signalement. **Fait localement :** `scripts/run_checks.py` exécute la même boucle Ruff/compilation/tests/couverture que les jobs qualité, les workflows qualité/Flatpak suivent maintenant les dépendances de test et les modules de stockage/tracker, le smoke Flatpak échoue si GTK ou la suppression manquent, le rapport release reçoit un statut machine `passed` seulement après le smoke, la release taguée est précédée d’un job source (tests, couverture cœur, Ruff, compilation, pip-audit, Bandit), et les actions ainsi que l’image GNOME 50 sont épinglées sur des révisions vérifiées ; les paramètres GitHub et la décision sur la pause de la CI générale restent ouverts.
