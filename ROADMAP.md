...
la vérification. Le workflow Flatpak GitHub Actions `34250484669` a réussi sur
le commit `5db464b` le 8 septembre 2026 : build en ligne, rebuild sans
...
- Workflows Flatpak, Release, benchmark et CodeQL actifs. CodeQL a réussi sur le
  commit documentaire courant `782137b` ; le smoke Flatpak a réussi sur le
  commit code `5db464b` (run `34250484669`). Ces jobs ne remplacent pas les
...
- **Changements :** identifiants de session et de segment dans les résultats ; ordonnancement borné ; définition des états terminé/échoué/annulé/vide ; invalidation des résultats tardifs à l’abandon ou au remplacement du provider. Une erreur de A doit libérer B sans attente infinie. **Fait localement :** un tampon partagé libère les résultats asynchrones dans l’ordre de soumission tout en conservant des workers parallèles ; les callbacks de résultat et d’erreur portent `request_id`, le contrôleur ignore ceux qui ne sont plus actifs, la fixture provider reproduit le même contrat et le bandeau des segments est vidé à chaque nouvelle génération. L’annulation/abandon complet et la génération persistante de session restent ouverts.
