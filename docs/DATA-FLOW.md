...
The Rust application keeps speech local until the local VAD closes a completed
segment. The user must separately configure a provider key and confirm the
cloud boundary before that segment can leave the process.
...
| API key | Environment or saved settings | Used for authentication | Saved only when the user chooses |
...
The Rust request worker applies a timeout, rejects oversized responses, and
...
only, is disabled by default, is retention-limited, and can be cleared.

The Python/GTK and whisper.cpp files remain as transitional migration material.
Their historical data-flow documentation must not be read as a description of
the active Rust runtime.
