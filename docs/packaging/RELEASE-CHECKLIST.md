...
- [ ] Unit tests, Ruff, and Python compilation pass.
- [ ] Desktop file and AppStream metadata validate with no errors.
- [ ] Flatpak manifest builds from a clean checkout.
...
- [ ] Bundle installs into a clean user Flatpak installation.
- [ ] Installed `--version`, `--help`, and `--doctor --json` contracts pass.
- [ ] Installed permissions contain only Wayland, fallback X11/IPC,
  PulseAudio, and network.
- [ ] Desktop file, MetaInfo, icon, and executable exist inside `/app`.
- [ ] GTK window and first-run dialog remain open under Xvfb.
- [ ] Uninstall with data removal succeeds.
...
- [ ] Bundle filename includes the exact application version and architecture.
- [ ] SHA-256 checksum file is generated from the uploaded bundle.
- [ ] Release notes name the supported environments and permission boundary.
- [ ] Source tag, changelog, package version, MetaInfo release, and bundle output
  all match.
