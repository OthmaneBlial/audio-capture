...
1. `AudioCapture` reads 30 ms 16 kHz mono PCM frames into a fixed-size queue.
...
4. Results return to GTK through its idle queue and are appended to the current transcript.

No audio recording is persisted by the application. Segments are sent to Groq only when speech has been detected. The transcript remains in the GTK buffer until the user clears, copies, or exports it.
...
- Client timeouts are configured when supported by the installed Groq SDK. SDK retries are disabled so the app's queue remains predictable.
- Stop first signals capture, waits briefly for the processor, then flushes a final valid segment.

## Configuration boundary

Settings use the precedence `defaults < config file < environment`. The configuration file is atomically replaced and set to `0600`; its parent directory is set to `0700` where supported. `GROQ_API_KEY` overrides a stored key and is not copied into settings when the environment value is active.
